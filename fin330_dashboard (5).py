...
def apply_theme():
    ...
    <style>
        ...
        /* Apply font/color to sidebar text but NOT to icon spans — the wildcard
           was overriding Material Symbols and turning icons into ligature text */
        ...
        /* Preserve Material Symbols on ALL sidebar icon spans */
        ...
        h1, h2, h3, h4, p, div, label, input, button, td, th {{
            font-family: {FONT} !important;
        }}
        ...
        .t-price {{ color: #BBBBBB;   margin-left: 7px; }}
        .t-up    {{ color: {C_GREEN}; margin-left: 5px; font-size: 0.85rem; }}
        .t-down  {{ color: {C_RED};   margin-left: 5px; font-size: 0.85rem; }}
        ...
        /* These use Material Symbols Rounded ligatures — must NOT inherit Calibri */
...
def inject_js():
    """
    Inject JavaScript via st.components.v1.html — the only way to run JS in Streamlit.
    st.markdown <script> tags are stripped by Streamlit's sanitizer and never execute.

    This does two things:
      1. Auto-collapse the sidebar when Run Analysis is clicked.
      2. Walk up into the parent frame (Streamlit renders components in iframes)
         to find and click the sidebar collapse button.
    """
    components.html(
        """
        <script>
        (function() {
            // We are inside a sandboxed iframe. Walk up to the top-level document.
            var topDoc = window.parent.document;

            function collapseSidebar() {
                // Streamlit's sidebar close button: button inside the sidebar header area
                // It has data-testid="baseButton-headerNoPadding" when sidebar is open.
                var btn = topDoc.querySelector('button[data-testid="baseButton-headerNoPadding"]');
                if (btn) {
                    btn.click();
                }
            }
            ...
            function attachRunListeners() {
                var buttons = topDoc.querySelectorAll('.stButton > button');
                buttons.forEach(function(btn) {
                    var txt = (btn.innerText || btn.textContent).trim().toUpperCase();
                    if (txt === 'RUN ANALYSIS' && !btn.dataset.bbgAttached) {
                        btn.dataset.bbgAttached = 'true';
                        btn.addEventListener('click', function() {
                            // 200ms delay: let Streamlit register the click before collapsing
            ...
            // Observe the top-level document for DOM changes (Streamlit re-renders on every run)
            ...
            // Initial attach
            setTimeout(attachRunListeners, 300);
        })();
        </script>
        """,
        height=0,   # zero height — invisible utility component
    )
...
def render_ticker_tape():
    ...
    for item in data:
        ...
        items_html += (
            ...
            f'</span>'
            f'<span style="color:#333; font-size:0.8rem; vertical-align:middle;">|</span>'
        )
...
def calc_performance_metrics(portfolio_returns, benchmark_returns,
...
def chart_cumulative_returns(portfolio_cumulative, benchmark_cumulative, benchmark):
    ...
    ax.plot(portfolio_cumulative.index, portfolio_cumulative,
            label="Portfolio", color=CH_PORT,  linewidth=2)
    ax.plot(benchmark_cumulative.index, benchmark_cumulative,
            label=benchmark,   color=CH_BENCH, linewidth=1.5, linestyle="--")
...
def chart_individual_returns(returns, tickers):
    ...
    ax.bar(individual_returns.index, individual_returns.values,
...
def ui_stock_analysis(ticker: str):
    ...
    ann_return = calc_annualized_return(df)

    col1, col2, col3 = st.columns(3)
    col1.metric("Annualized Return",     f"{ann_return:.2f}%")
...
def ui_portfolio_dashboard(tickers: list, weights: list, benchmark: str):
...
def main():
    ...
    if section == "Stock Analysis":
        ...
        tickers_input = st.sidebar.text_input(
            "Tickers (comma-separated, any number)",
            "AAPL, MSFT, JPM, AMZN, NVDA"
        )
        tickers = [t.strip().upper() for t in tickers_input.split(",") if t.strip()]

        benchmark = st.sidebar.text_input("Benchmark ETF", "SPY").upper()
        st.sidebar.markdown(f"<hr style='border-top:1px solid {C_BORDER}; margin:0.4rem 0'>",
                            unsafe_allow_html=True)

        if tickers:
            st.sidebar.markdown(
                f"<p style='color:{C_MUTED}; font-size:0.7rem; text-transform:uppercase; "
                f"letter-spacing:0.1em; font-family:{FONT};'>Weights (must sum to 1.00)</p>",
                unsafe_allow_html=True
            )
            ...
            if abs(weight_sum - 1.0) > 0.01:
                st.sidebar.warning(f"Weights sum to {weight_sum:.2f} — must equal 1.00")
            else:
                st.sidebar.success("Weights sum to 1.00")
        else:
            weights = []
